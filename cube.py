...
def face_to_string(face):
    return "".join([FACE_MAP[x] for x in face.flatten()])

# 得到魔方的字符串表示用于输入kociemba求解
def encode_cube(faces):
    u = face_to_string(faces[0])
    r = face_to_string(faces[3])
    f = face_to_string(faces[4])
    d = face_to_string(faces[1])
    l = face_to_string(faces[2])
    b = face_to_string(faces[5])
    return u + r + f + d + l + b
